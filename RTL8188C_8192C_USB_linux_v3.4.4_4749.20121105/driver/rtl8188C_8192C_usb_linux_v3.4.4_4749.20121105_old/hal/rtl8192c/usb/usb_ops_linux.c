...
static int recvbuf2recvframe(_adapter *padapter, _pkt *pskb)
{
	...
	printk(KERN_EMERG "\n recvbuf2recvframe");	//Agostino Polizzano
	...
	do{		
		...
		/*
		network_header = (struct iphdr *)skb_network_header(precvframe->u.hdr.pkt);
		if (network_header != NULL) {
			printk(KERN_EMERG "network_header not null!\n");
			if (network_header->protocol == 1) { //ICMP
				//if (network_header->saddr == _ip_address) {
					printk(KERN_EMERG "RX_STATUS: packet sent from %pI4 (packet type: ICMP) to %pI4\n", &network_header->saddr, &network_header->daddr);
				//}
			}
		}
		*/
...
void rtl8192cu_recv_tasklet(void *priv)
{
	...
	struct iphdr *network_header;				//Agostino Polizzano
	printk(KERN_EMERG "\n rtl8192cu_recv_tasklet");	//Agostino Polizzano
	...
	while (NULL != (pskb = skb_dequeue(&precvpriv->rx_skb_queue)))
	{
		...
		/* start: Agostino Polizzano */
		network_header = (struct iphdr *)skb_network_header(pskb);
		if (network_header != NULL) {
			if (network_header->protocol == 1) { //ICMP
				//if (network_header->saddr == _ip_address) {
					printk(KERN_EMERG "RX_STATUS: packet sent from %pI4 (packet type: ICMP) to %pI4\n", &network_header->saddr, &network_header->daddr);
